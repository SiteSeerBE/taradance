        ...
        return (
            <div className="container mt1">
                <article>
                    ...
                    <p>U moet ingelogd zijn om lessen te bestellen. <Link href="/dashboard">Naar aanmeldpagina</Link>.</p>
                </article>
            </div>
        );
    }
...
const OrderStepTwo = async ({ currentOrder, logtoId }: { currentOrder: OrderWithProduct[], logtoId: string }) => {
    const childAccounts = await prisma.user.findMany({
        where: {
            OR: [
                { parentAccount: { logtoId } },
                { logtoId },
            ],
        },
        orderBy: {
    ...
    return (
        <div className="container mt1">
            <OrderAsignList currentOrder={currentOrder} childAccounts={childAccounts} />
        </div>
    );
}
