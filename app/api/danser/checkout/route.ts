...
export async function POST(request: Request) {
  const payedByUserId = await getUserIdForRole([
    RoleType.DANSER,
    RoleType.ADMIN,
  ]);
  if (!payedByUserId) {
  ...
  if (order.length === 0) {
    return new Response(JSON.stringify({ error: "Niets te bestellen" }), {
      status: 400,
    });
  }
  ...
  if (order.some((item) => !assignments[item.id])) {
    return new Response(
      JSON.stringify({ error: "Niet alle deelnemers zijn toegewezen" }),
      { status: 400 },
    );
  }
  ...
  if (order.some((item) => !allowedUserIds.has(assignments[item.id]))) {
    return new Response(
      JSON.stringify({ error: "Ongeldige deelnemer toegewezen" }),
      { status: 400 },
    );
  }
  ...
  for (const item of order) {
    ...
    if (seen.has(key)) {
      return new Response(
        JSON.stringify({ error: "Dubbele inschrijving voor een deelnemer" }),
        { status: 400 },
      );
    }
    seen.add(key);
  ...
  const isFree = (item: (typeof order)[number]) =>
  ...
  try {
    ...
    const payment = await mollieClient.payments.create({
      ...
      description: `Scoil Rince Celtus Belgium danslessen (${paidItems.length} item${paidItems.length > 1 ? "s" : ""})`,
      ...
      ...(isLocal
        ? {}
        : { webhookUrl: `${baseUrl}/api/danser/checkout/webhook` }),
    ...
    return new Response(
      JSON.stringify({ error: "Betaling starten is mislukt" }),
      { status: 500 },
    );
  }
}
